...
def select_model(model_name, num_classes=100):  # Changed default to 100 classes
...
def main():
    ...
    net = select_model(args.model, num_classes=100).to(device)  # Explicitly set to 100 classes
    optimizer = select_optimizer(args.optimizer, net)
    criterion = nn.CrossEntropyLoss().to(device)
    print(f"Using model: {args.model} with optimizer: {args.optimizer} on CIFAR-100")
